...
class MainMenuStateSecond extends MusicBeatState
{
	var curSelected:Int = 0;
	...
	#if !switch
	var optionShit = ['credits', 'mods']; // no more txt i will add it in a later update
	//var optionShit:Array<String> = [optionsShit];
	#else
	var optionShit:Array<String> = ['credits'];
	#end
	...
	override function create()
	{
		...
		gradientBar = FlxGradient.createGradientFlxSprite(Math.round(FlxG.width), 512, [0x00ff0000, 0x55AE59E4, 0xAA19ECFF], 1, 90, true);
		//gradientBar = FlxGradient.createGradientFlxSprite(Math.round(FlxG.width), 512, [0xBBB8B4, 0xBBB8B4, 0xBBB8B4], 1, 90, true);
		gradientBar.y = FlxG.height - gradientBar.height;
		add(gradientBar);
		gradientBar.scrollFactor.set(0, 0);
		...
		var tex = Paths.getSparrowAtlas('FNF_main_menu_assets_page2');
		...
		var versionShit:FlxText = new FlxText(305, FlxG.height - 696, 0, "Press K To Go To Page 1", 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
					...
					menuItems.forEach(function(spr:FlxSprite)
					{
						...
						else
						{
							FlxFlicker.flicker(spr, 1, 0.06, false, false, function(flick:FlxFlicker)
							{
								...
								{
									case 'credits':
										//FlxG.switchState(new CreditsNew());
										trace("Hehe");
									case 'mods':
										FlxG.switchState(new ImageMovement());
										trace("Hehe");
