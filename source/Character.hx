...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'nerd':
				// DAD ANIMATION LOADING CODE
				tex = Paths.getSparrowAtlas('NerdAssets');
				frames = tex;
				animation.addByPrefix('idle', 'Idle', 24);
				animation.addByPrefix('singUP', 'Up', 24);
				animation.addByPrefix('singRIGHT', 'Right', 24);
			...
			case 'parents-christmas':
				...
				case 'tankman':
					...
					animation.addByPrefix('singLEFT-alt', 'Tankman UP note instance 1', 24); // we need this for the ugh animations
			...
			/*case 'bf-holding-gf':
				frames = Paths.getSparrowAtlas('characters/bf-holding-gf');
				if (isPlayer) {
					animation.addByPrefix('singLEFT','BF NOTE LEFT0', 24, false);
					animation.addByPrefix('singRIGHT', 'BF NOTE RIGHT0', 24, false);
				    animation.addByPrefix('singLEFTmiss', 'BF NOTE LEFT MISS',24, false);
				...
				playAnim('idle');*/
		/*	case 'gf-tankmen':
				// GIRLFRIEND TANKMEN CODE
				tex = Paths.getSparrowAtlas('characters/gf-tankmen');
				frames = tex;
		...
		if (isPlayer)
		{
			...
			/*if (!curCharacter.startsWith('bf-holding-gf'))
				{
					// var animArray
					var oldRight = animation.getByName('singRIGHT').frames;
					animation.getByName('singRIGHT').frames = animation.getByName('singLEFT').frames;
